...
function Key({keyVal, bigKey, ope}) {
  ...
  const selectLetter = () => {
    ...
    if(keyVal === "ENTER"){
      ...
      if(board[currAttempt.attempt][1] === '+'){
        newBoard[currAttempt.attempt][3] = parseInt(board[currAttempt.attempt][0]) + parseInt(board[currAttempt.attempt][2]);
      } else if(board[currAttempt.attempt][1] === '-'){
        newBoard[currAttempt.attempt][3] = parseInt(board[currAttempt.attempt][0]) - parseInt(board[currAttempt.attempt][2]);
      } else if(board[currAttempt.attempt][1] === '*'){
        newBoard[currAttempt.attempt][3] = parseInt(board[currAttempt.attempt][0]) * parseInt(board[currAttempt.attempt][2]);
      } else if(board[currAttempt.attempt][1] === '/'){
        newBoard[currAttempt.attempt][3] = Math.round(parseInt(board[currAttempt.attempt][0]) / parseInt(board[currAttempt.attempt][2]));
      }
      setBoard(newBoard);
  ...
  return (
    <div className='key' id = {bigKey && "big"} onClick={ decision == true ? selectLetter : null }>{keyVal}</div>
  )
}
